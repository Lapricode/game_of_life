...
class game_of_life():
    ...
    def start_interface(self):
        ...
        self.grid_size = 0.8 * self.grid_height
        ...
        self.grid_background = tk.Frame(self.grid_labels_background, width = int(self.grid_size), height = int(self.grid_size))
        self.grid_background.grid(row = 0, column = 0, columnspan = 4, sticky = tk.NSEW)
        self.grid_background.grid_propagate(0)
        self.grid_matrix = []
        self.grid_cells = []
        ...
        self.button_size = int(self.grid_size / (self.right_column + 1 - self.left_column))
        self.grid_information_string = tk.StringVar()
        self.grid_information = tk.Label(self.grid_labels_background, textvariable = self.grid_information_string, font = 'Arial 10 bold', bd = 2, relief = 'solid')
        self.grid_information.grid(row = 1, column = 0, sticky = tk.NSEW)
        self.grid_colors_label = tk.Label(self.grid_labels_background, text = 'Colors:', font = 'Arial 15 bold', bg = 'grey', fg = 'purple')
        ...
        self.control_grid_limits_button.grid(row = 1, column = 3, sticky = tk.NSEW)
        for i in range(self.rows):
            self.grid_matrix_row = []
            self.grid_cells_row = []
            for j in range(self.columns):
                self.grid_matrix_row.append(0)
                self.grid_cells_row.append(button(self.grid_background, self.button_size, i, j))
            self.grid_matrix.append(self.grid_matrix_row)
        ...
        for i in range(self.up_row, self.down_row + 1):
            for j in range(self.left_column, self.right_column + 1):
                self.grid_cells[i][j].frame_button.grid(row = self.grid_cells[i][j].row, column = self.grid_cells[i][j].column, sticky = tk.NSEW)
                self.grid_cells[i][j].button.grid()
        self.graf_history = []
        ...
        self.grid_situation_reveal = tk.Canvas(self.grid_situation_reveal_background, width = int(self.grid_situation_reveal_width), height = int(self.grid_situation_reveal_height), bg = self.colors_list[-1])
        ...
        for i in range(self.rows):
            for j in range(self.columns):
                if self.grid_cells[i][j].control_press == 'unpressed':
                    self.grid_cells[i][j].button.configure(bg = self.colors_list[-1])
                elif self.grid_cells[i][j].control_press == 'pressed':
                    self.grid_cells[i][j].button.configure(bg = self.colors_list[self.grid_cells[i][j].choose_color])
    ...
    def grid_up(self):
        if self.up_row != 0:
            for i in range(self.left_column, self.right_column + 1):
                self.grid_cells[self.down_row][i].button.grid_remove()
                self.grid_cells[self.down_row][i].frame_button.grid_remove()
            self.up_row -= 1
            self.down_row -= 1
            for i in range(self.left_column, self.right_column + 1):
                self.grid_cells[self.up_row][i].frame_button.grid(row = self.grid_cells[self.up_row][i].row, column = self.grid_cells[self.up_row][i].column, sticky = tk.NSEW)
                self.grid_cells[self.up_row][i].button.grid()
            self.grid_information_string_inform()
    def grid_left(self):
        if self.left_column != 0:
            for i in range(self.up_row, self.down_row + 1):
                self.grid_cells[i][self.right_column].button.grid_remove()
                self.grid_cells[i][self.right_column].frame_button.grid_remove()
            self.left_column -= 1
            self.right_column -= 1
            for i in range(self.up_row, self.down_row + 1):
                self.grid_cells[i][self.left_column].frame_button.grid(row = self.grid_cells[i][self.left_column].row, column = self.grid_cells[i][self.left_column].column, sticky = tk.NSEW)
                self.grid_cells[i][self.left_column].button.grid()
            self.grid_information_string_inform()
    def grid_right(self):
        if self.right_column != 99:
            for i in range(self.up_row, self.down_row + 1):
                self.grid_cells[i][self.left_column].button.grid_remove()
                self.grid_cells[i][self.left_column].frame_button.grid_remove()
            self.left_column += 1
            self.right_column += 1
            for i in range(self.up_row, self.down_row + 1):
                self.grid_cells[i][self.right_column].frame_button.grid(row = self.grid_cells[i][self.right_column].row, column = self.grid_cells[i][self.right_column].column, sticky = tk.NSEW)
                self.grid_cells[i][self.right_column].button.grid()
            self.grid_information_string_inform()
    def grid_down(self):
        if self.down_row != 99:
            for i in range(self.left_column, self.right_column + 1):
                self.grid_cells[self.up_row][i].button.grid_remove()
                self.grid_cells[self.up_row][i].frame_button.grid_remove()
            self.up_row += 1
            self.down_row += 1
            for i in range(self.left_column, self.right_column + 1):
                self.grid_cells[self.down_row][i].frame_button.grid(row = self.grid_cells[self.down_row][i].row, column = self.grid_cells[self.down_row][i].column, sticky = tk.NSEW)
                self.grid_cells[self.down_row][i].button.grid()
            self.grid_information_string_inform()
    def grid_rotate(self, rotation_direction):
        ...
        for i in range(self.up_row, self.down_row + 1):
            ...
            for j in range(self.left_column, self.right_column + 1):
                ...
                self.grid_cells_info_element.append(self.grid_cells[i][j].button['bg'])
        ...
        for i in range(self.up_row, self.down_row + 1):
            for j in range(self.left_column, self.right_column + 1):
                ...
                self.grid_cells[self.y_new][self.x_new].button.configure(bg = self.grid_cells_info[self.y_old][self.x_old][2])
    ...
    def grid_zoom_in(self):
        if self.right_column - self.left_column != 1:
            ...
            self.up_row += 1
            self.down_row -= 1
            self.new_button_size = int(self.grid_size / (self.right_column + 1 - self.left_column))
            for i in range(self.rows):
                for j in range(self.columns):
                    self.grid_cells[i][j].button.configure(width = self.new_button_size, height = self.new_button_size)
                    self.grid_cells[i][j].frame_button.configure(width = self.new_button_size, height = self.new_button_size)
            self.grid_information_string_inform()
        ...
        for row in range(up_row, down_row + 1):
            for column in range(left_column, right_column + 1):
                if self.grid_cells[row % self.rows][column % self.columns].control_press == 'pressed':
                    self.x = (column - left_column + 0.5) * self.size_x
                    self.y = (row - up_row + 0.5) * self.size_y
    ...
    def adjust_to_grid(self):
        for i in range(self.up_row, self.down_row + 1):
            for j in range(self.left_column, self.right_column + 1):
                self.grid_cells[i][j].button.grid_remove()
                self.grid_cells[i][j].frame_button.grid_remove()
...
class button():
    ...
    def __init__(self, grid_background, button_size, row, column):
        ...
        self.frame_button = tk.Frame(self.grid_background, bd = 1, width = self.button_size, height = self.button_size, bg = 'orange')
        self.frame_button.grid_propagate(0)
        self.button = tk.Label(self.frame_button, bg = '#006fff', relief = 'solid', width = self.button_size, height = self.button_size, compound = 'center')
            ...
            if game_of_life.generations_string.get() != '0' and game_of_life.simulation_state == 'stop_simulation' and game_of_life.recording_play_stop_state == 'stop':
                ...
                if game_of_life.set_chosen_position == 'on':
                    ...
                    if self.control_press == 'unpressed':
                        self.choose_color = 0
                        self.button.configure(bg = game_of_life.colors_list[0])
                        ...
                        self.choose_color = 0
                        self.button.configure(bg = game_of_life.colors_list[-1])
                        self.control_highlight = 0
                        self.control_press = 'unpressed'
                        if game_of_life.simulation_state == 'stop_simulation' and game_of_life.recording_play_stop_state == 'stop':
                            if game_of_life.grid_reveal_mode_button['text'] == 'Auto':
    ...
    def unhighlight_button(self, event):
        try:
            if self.control_highlight == 0:
                ...
                self.button.configure(bg = game_of_life.colors_list[self.choose_color])
        except:
            pass
